...
def home(request):
    ...
    try:
        if validating.check_in.date() == datetime.date.today() and validating.check_out:
            ...
            notify = {"check_in":validating.check_out,"check_out":validating.check_out,"check_in_image":validating.check_in_image}
...
def check_in_page(request):
        ...
        try:
            ...
            saving.check_in_image.save(f"{request.user}' '{date_time}.png", File(open(f"/home/raviteja/attedance/{request.user}.png", "rb")))
...
def check_out(request):
        ...
        saving.check_out_image.save(f"{request.user}' '{date_time}.png", File(open(f"/home/raviteja/attedance/{request.user}.png", "rb")))
        #saving.save()
